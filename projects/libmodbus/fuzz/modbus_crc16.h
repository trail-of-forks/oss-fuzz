/* modbus_crc16.h - Standalone CRC-16 for fuzzing
 * Extracted from libmodbus/src/modbus-rtu.c
 *
 * Copyright 2022 Google LLC
 * Licensed under the Apache License, Version 2.0
 */
#ifndef MODBUS_CRC16_H
#define MODBUS_CRC16_H

#include <stdint.h>
#include <stddef.h>

static const uint8_t table_crc_hi[] = {
    
};

static const uint8_t table_crc_lo[] = {
    
};

static inline uint16_t modbus_crc16(const uint8_t *buffer, size_t length) {
    uint8_t crc_hi = 0xFF, crc_lo = 0xFF;
    while (length--) {
        unsigned int i = crc_lo ^ *buffer++;
        crc_lo = crc_hi ^ table_crc_hi[i];
        crc_hi = table_crc_lo[i];
    }
    return (crc_hi << 8) | crc_lo;
}

static inline void modbus_crc16_append(uint8_t *buffer, size_t payload_len) {
    uint16_t crc = modbus_crc16(buffer, payload_len);
    buffer[payload_len] = crc & 0xFF;      /* Low byte first */
    buffer[payload_len + 1] = crc >> 8;    /* High byte second */
}

#endif /* MODBUS_CRC16_H */
