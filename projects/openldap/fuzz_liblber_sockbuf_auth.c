/*
 * Harness: fuzz_liblber_sockbuf_auth
 *
 * TIER:    2 (BER wire-format parsing, shared between client and server)
 * TESTS:   BER framing and LDAP message parsing via Sockbuf (authenticated size limit)
 * PATH:    Sockbuf -> ber_get_next() -> ber_scanf() message parsing
 * CONFIG:  sb_max_incoming = 16777215 (16MB - matches slapd authenticated default)
 *
 * Tests BER framing and decoding through Sockbuf with the larger
 * authenticated message size limit. After successful bind, slapd
 * increases the limit from 256KB to 16MB, exposing additional attack
 * surface in the 256KB-16MB message size range.
 *
...
/* Authenticated connection limit: 16MB - 1 (matches SLAP_MAX_INCOMING_AUTH) */
#define MAX_INCOMING_AUTH ((1 << 24) - 1)  /* 16777215 */

/* For fuzzing efficiency, limit actual input size */
